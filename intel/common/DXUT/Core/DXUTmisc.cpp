...
typedef HRESULT                         ( WINAPI* LPDIRECT3DCREATE9EX )( UINT, IDirect3D9** );
...
static LPDIRECT3DCREATE9EX                  s_DynamicDirect3DCreate9Ex = NULL;
...
IDirect3D9* WINAPI DXUT_Dynamic_Direct3DCreate9( UINT SDKVersion )
{
    if(DXUTGetD3D9Ex())
    {
        if( DXUT_EnsureD3D9APIs() && s_DynamicDirect3DCreate9Ex != NULL )
        {
            IDirect3D9* pD3D = NULL;
            HRESULT hr;
            V(s_DynamicDirect3DCreate9Ex( SDKVersion, &pD3D));
            return pD3D;
        }
        else
            return NULL;
